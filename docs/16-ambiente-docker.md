...
Definição do ambiente containerizado — simples, reproduzível e com **paridade entre desenvolvimento e avaliação**. Esta é a **estrutura definida**; a implementação (Dockerfiles/compose) é etapa posterior. Legenda em [08-arquitetura-geral.md](08-arquitetura-geral.md#legenda-de-origem-das-decisões).

## Objetivos

🟦 O ambiente deve permitir:
- Desenvolver e **testar** o projeto localmente (back, front e banco integrados).
- **Clone-and-run:** o avaliador clona e executa com **mínima configuração** (idealmente um comando).
...
|---|---|---|---|
| **db** | `mysql:8.0` | Banco de dados; volume nomeado para persistência; healthcheck. | 3306 |
| **backend** | `php:8.4` (+ Composer, extensões `pdo_mysql`, `mbstring`, `bcmath`, `zip`) | API Laravel via `php artisan serve --host=0.0.0.0 --port=8000`. | 8000 |
| **frontend** | `node:20` | SPA React (Vite) via `npm run dev -- --host`. | 5173 |
...
├── .gitignore
├── backend/
│   ├── Dockerfile             → imagem PHP 8.3 + Composer + extensões
│   ├── .dockerignore
│   ├── .env.example           → env do Laravel
│   └── (aplicação Laravel)
├── frontend/
│   ├── Dockerfile             → imagem Node 20 (Vite)
│   ├── .dockerignore
│   ├── .env.example           → VITE_API_URL etc.
│   └── (aplicação React)
└── docker/                    → (opcional) scripts de init/entrypoint
```
...
- Ao final: **API** em `http://localhost:8000`, **App** em `http://localhost:5173`.
...
```
Ver [14-estrategia-de-testes.md](14-estrategia-de-testes.md). A base de teste do MySQL roda no mesmo serviço `db` (banco dedicado) para manter a paridade de engine.
