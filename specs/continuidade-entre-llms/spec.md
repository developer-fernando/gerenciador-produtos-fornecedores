...
| Campo | Valor |
|---|---|
| **Status** | Concluído (C-T1..C-T4; cold-start verificado) |
...
- [x] **C-T3** — Generalizar a mecânica para tool-agnóstica nos **três** arquivos: `specs/README.md`, `AGENTS.md` e `specs/_templates/validation.md` (verificador = contexto novo; subagente como opção) — _grep por "subagente" nos três não retorna nenhuma pressuposição obrigatória; spec futura copiada do template já nasce tool-agnóstica._
- [x] **C-T4** — Fechar a verificação documental: cold-start simulado por um verificador em **contexto novo** que recebe só o repositório e responde "feature atual + próximo ticket" — _aprovado; registrado no [validation.md](./validation.md#verificação-de-cold-start-c-t4--aprovado). Critérios do PRD atendidos._

## 8. Definition of Done

- [x] Todos os tickets concluídos.
- [x] Cold-start simulado por um verificador em contexto novo identifica fase/feature/próximo ticket **sem** a conversa anterior.
- [x] `ESTADO.md` com seções fixas e tabela de progresso coerente com `specs/`.
- [x] `AGENTS.md` §0 + ponteiro no `README.md`.
- [x] Mecânica tool-agnóstica aplicada nos **três** arquivos (`specs/README.md`, `AGENTS.md`, `specs/_templates/validation.md`): grep por "subagente" não retorna nenhuma etapa obrigatória.
- [x] Sem duplicação de `docs/`; tudo em markdown versionado.
