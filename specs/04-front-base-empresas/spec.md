...
- [x] **04-T2** — `styles/` (tokens da paleta + global, foco visível) + `app/` (providers Query+Router, rota `/`, layout com `Header`/logo) — _app renderiza o shell com identidade Horizon (verificado no navegador: header preto/amarelo, marca com fallback textual, redirect `/`→`/empresas`); build/test/lint verdes._
- [x] **04-T3** — `lib/`: `http.ts` (axios + baseURL + `Accept`/`Content-Type`), `errors.ts` (normalização), `queryClient.ts` (feito na T2) + testes de `errors.ts` (422 validação/422 regra/409/404/500/rede/não-axios) — _10 testes verdes; build/lint ok._
- [x] **04-T4** — `shared/`: `format.ts` (+ testes) e componentes base (`Table`/`Pagination`, `StatusBadge`, `Modal`, `ConfirmDialog`, `FormField`, `Button`, `Toast`) — _8 testes de formatters + 4 do `StatusBadge` (inclui o caso combinado Inativo+Excluído → "Excluído", F4) verdes; build/lint limpos._
- [x] **04-T5** — `features/empresas` camada de dados: `types.ts`, `api.ts`, `hooks.ts` (query + 7 mutações com invalidação de `['empresas']`) — _tsc compila; hooks/queries montados (exercitados via MSW em T6–T8)._
