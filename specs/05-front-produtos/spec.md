...
- [x] **05-T2** — Base + `features/produtos` camada de dados: `AcoesPermitidas` movido para `shared/types.ts` (re-export em `empresas/types.ts` mantém os imports existentes); `types.ts`, `api.ts` (com `listarEmpresasAptas()` iterando páginas), `hooks.ts` (`useProdutos`, `useProdutoMutations`, `useEmpresasAptas`) — _tsc compila; suíte da 04 (39) verde após o move; lint ok._
